...
AudioInputI2Sslave       i2s;
AudioAnalyzeFFT1024      fft_A;
AudioAnalyzeFFT1024      fft_B;

AudioConnection          patchCord1(i2s, 0, fft_A, 0);
AudioConnection          patchCord2(i2s, 1, fft_B, 0);
...
// Determine a new destination height
    for (x=0; x < matrix_width; x++) {
      yCurr[x] = yTarg[x];
      yTarg[x] = 0;
      level[x] = fft_A.read(freqBin, freqBin + frequencyBinsHorizontal[x] - 1);
